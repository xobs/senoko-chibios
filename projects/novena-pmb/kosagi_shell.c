...
/**
 * @file    kosagi_shell.c
 * @brief   Simple CLI shell code with Kosagi compatibility.
 *
 * @addtogroup SHELL
 * @{
 */
...
static bool_t cmdexec(const ShellCommand *scp, BaseSequentialStream *chp,
  ...
  while (scp->sc_name != NULL) {
    if (_strcasecmp(scp->sc_name, name) == 0) {
...
static msg_t shell_thread(void *p) {
  ...
  while (TRUE) {
    ...
    if (cmd != NULL) {
      ...
      else if (_strcasecmp(cmd, "help") == 0) {
        if (n > 0) {
          usage(chp, "help");
          continue;
        }
